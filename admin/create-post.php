 ...
 *
 * Create a new blog post with full TinyMCE rich text editor.
 * Includes cover image upload/crop and gallery management.
 *
 * @package IrtiJa
 * @version 1.0
 */
...
try {
    ...
    db_log_error('Failed to fetch categories for create post', ['error' => $e->getMessage()]);
}
    ...
    if (!isset($_POST['csrf_token']) || !admin_validate_csrf($_POST['csrf_token'])) {
        ...
        if (!empty($slug)) {
            ...
            if ((int)$existing > 0) {
                ...
                while ((int)db_fetch_column('SELECT COUNT(*) FROM posts WHERE slug = :slug', ['slug' => $newSlug]) > 0) {
                    $counter++;
                    $newSlug = $slug . '-' . $counter;
                }
                $slug = $newSlug;
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($image) {
                    ...
                    if (imagewebp($image, $webpPath, 75)) {
                        $galleryPaths[] = 'uploads/gallery/' . basename($webpPath);
                    } else {
                        if (move_uploaded_file($file['tmp_name'], $targetPath)) {
                            $galleryPaths[] = 'uploads/gallery/' . $filename;
                        }
        ...
        if (count($galleryPaths) > 10) {
            $errors[] = 'Maximum 10 gallery images allowed.';
            $galleryPaths = array_slice($galleryPaths, 0, 10);
        }
        ...
        if (empty($errors)) {
            try {
                ...
                $success = true;
                $successMessage = 'Post created successfully! <a href="edit.php?id=' . $postId . '">Edit this post</a> or <a href="posts.php">view all posts</a>.';
                ...
                db()->rollBack();
                db_log_error('Failed to create post', ['error' => $e->getMessage()]);
                $errors[] = 'Database error: Could not save the post.';
            }
        } else {
            // If errors, repopulate form data
            $postData = [
                'title' => $title,
                'slug' => $slug,
                'display_date' => $displayDate,
...
<head>
    ...
    <title>Create Post · IrtiJa Admin</title>
    ...
    <script src="https://cdn.tiny.cloud/1/sv41qlo6d2qx14rohe42dhmnrxvie7vwy3ph42g62i8tqk6g/tinymce/8/tinymce.min.js" referrerpolicy="origin" crossorigin="anonymous"></script>

    <style>
        /* ============================================================
           ADMIN CREATE POST — Full styling
           ============================================================ */
        * {
            margin: 0;
            padding: 0;
        ...
        .btn-secondary:hover {
            transform: translateY(-2px);
            box-shadow: 0 8px 28px rgba(212, 168, 83, 0.20);
        }
        ...
        .cropper-modal .cropper-container img {
            max-width: 100%;
            display: block;
        }
...
<div class="admin-wrapper">
    ...
    <aside class="admin-sidebar" role="navigation" aria-label="Admin navigation">
        ...
        <div class="nav-section">
            ...
            <a href="create-post.php" class="nav-link active"><i class="fas fa-plus-circle"></i> New Post</a>
    ...
    <main class="admin-main" role="main">
        ...
        <?php if (isset($success) && $success): ?>
            <div class="message success" role="status">
                <?php echo $successMessage ?? 'Post created successfully!'; ?>
            </div>
        <?php endif; ?>
        ...
        <form class="form-card" method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div class="help-text">Write the full blog post content. Supports rich formatting.</div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="cover-upload-area">
                    ...
                    <div class="help-text">Upload a cover image (JPEG, PNG, GIF, WebP). You can crop it after upload.</div>
                </div>
            </div>

            <!-- Gallery Images -->
            <div class="form-group">
                <label>Gallery Images (1–10)</label>
                <div class="gallery-upload-area">
                    ...
                    <div class="help-text">Upload up to 10 images for the gallery. Supported formats: JPEG, PNG, GIF, WebP.</div>
                </div>
            </div>
...
<script>
    // --- TinyMCE initialization with your API key ---
    tinymce.init({
        selector: '#content',
        height: 400,
        menubar: false,
